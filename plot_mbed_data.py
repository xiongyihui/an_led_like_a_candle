...
class SerialThread(QThread):
    ...
    def run(self):
        ...
        for p in list_ports.comports():
            print(p)
            if p[2].upper().startswith('USB VID:PID'):
                port = p[0]
                break
        ...
        while not self.exit:
            try:
                ...
                voltage = int(line) * 3.3 / (2**16)
                print(voltage)
                data[data_index] = voltage
                data_index += 1
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('LED Voltage Monitor')
        self.resize(800, 500)
